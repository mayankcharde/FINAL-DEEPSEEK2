...
// Mistral is used for AI responses (initialized via mistral.config.js)
if (!process.env.MISTRAL_API_KEY) {
  console.error("MISTRAL_API_KEY not found in environment variables");
} else {
  console.log("Mistral AI configured successfully");
}
...
// Helper — Get Detailed Mistral Answer
async function getMistralAnswer(imageData, userQuestion) {
  try {
...
router.post("/analyze", protect, async (req, res) => {
  try {
    ...
    if (!process.env.MISTRAL_API_KEY) {
      ...
      return res
        .status(500)
        .json({ error: "Server configuration error: Missing Mistral API key" });
    }
    ...
    // Step 2: Generate a detailed answer using Mistral
    console.log("Calling Mistral API...");
    const answer = await getMistralAnswer({ tags }, userQuestion);
    ...
    } else if (err.message.includes("Mistral")) {
